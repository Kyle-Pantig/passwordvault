...
export interface SafariProps extends SVGProps<SVGSVGElement> {
  url?: string;
  imageSrc?: string;
  imageSrcs?: string[]; // New prop for multiple images
  texts?: string[]; // New prop for text content for each image
  videoSrc?: string;
  width?: number;
  height?: number;
  mode?: SafariMode;
  autoRotate?: boolean; // New prop to enable auto-rotation
  rotationInterval?: number; // New prop to control rotation speed (in ms)
  slideDirection?: 'horizontal' | 'vertical'; // New prop for slide direction
  animationDuration?: number; // New prop for animation duration
  showText?: boolean; // New prop to show/hide text overlays
  textPosition?: 'top' | 'bottom' | 'center'; // New prop for text position
}
...
export function Safari({
  ...
  const images = imageSrcs && imageSrcs.length > 0 ? imageSrcs : (imageSrc ? [imageSrc] : []);
  
  // Determine which texts to use
  const textArray = texts && texts.length > 0 ? texts : (images.map((_, index) => `Image ${index + 1}`));
  ...
  // Get current image and text
  // const currentImage = images[currentImageIndex];
  // const currentText = textArray[currentImageIndex] || '';
  return (
    <svg
      width={width}
      height={height}
    ...
    >
      <g clipPath="url(#path0)">
        ...
        <g clipPath="url(#roundedBottom)">
          {images.map((image, index) => {
            const isHorizontal = slideDirection === 'horizontal';
            const offset = isHorizontal 
              ? (index - currentImageIndex) * 1200
              : (index - currentImageIndex) * 700;
            
            const x = isHorizontal ? 1 + offset : 1;
            const y = isHorizontal ? 52 : 52 + offset;
        ...
        {/* Animated text overlay with sliding effect */}
        {showText && (
          <g clipPath="url(#roundedBottom)">
            {textArray.map((text, index) => {
              ...
              return (
                <g key={index}>
                  ...
                  <rect
                    x={x}
                    y={textPosition === 'top' ? y : textPosition === 'center' ? y + 324 : y + 598}
                    width="1200"
                    height="80"
                    fill="rgba(0, 0, 0, 0.6)"
                    style={{
                      ...
                      transform: isHorizontal 
                        ? `translateX(${offset}px) scale(${index === currentImageIndex ? 1 : 0.95})` 
                        : `translateY(${offset}px) scale(${index === currentImageIndex ? 1 : 0.95})`,
                  ...
                  <text
                    x={x + 600}
                    y={textPosition === 'top' ? y + 48 : textPosition === 'center' ? y + 372 : y + 648}
                    textAnchor="middle"
                    fill="white"
                    fontSize="24"
                    fontFamily="Arial, sans-serif"
        ...
        {images.length > 1 && (
          <g>
            {images.map((_, index) => (
              <circle
                key={index}
                cx={600 + (index - (images.length - 1) / 2) * 20}
                cy={720}
                r="4"
                fill={index === currentImageIndex ? "#007AFF" : "rgba(255, 255, 255, 0.5)"}
                style={{
